...
const { extractQuality, axios } = require('./utils');
...
async function searchMovieByImdb(imdbId, token) {
    try {
...
async function searchSeriesByImdb(imdbId, token) {
    try {
        const response = await axios.get(`${EMBY_URL}/emby/Items`, {
            params: {
                AnyProviderIdEquals: `imdb.${imdbId}`,
...
async function getMovieStreams(imdbId) {
    ...
    if (!token) return [];
    const movies = await searchMovieByImdb(imdbId, token);
...
async function getSeriesStreams(imdbId, season, episode) {
    const token = await authenticate();
    if (!token) return [];
    const seriesList = await searchSeriesByImdb(imdbId, token);
...
module.exports = {
    ...
    async getStreams(type, meta, season, episode) {
        ...
        if (type === 'movie') return await getMovieStreams(imdbId);
        else return await getSeriesStreams(imdbId, season, episode);
    }
};
